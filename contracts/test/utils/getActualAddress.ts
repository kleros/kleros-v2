 ...
 */
export async function getActualAddress(
  network: string,
  contractName: string,
): Promise<string> {
  try {
    ...
    if (!deployment.default.address) {
      throw new Error(
        `No address found in deployment file for ${contractName} on ${network}`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `No deployment file found for ${contractName} on ${network}`,
      );
    }
    throw error;
  }
