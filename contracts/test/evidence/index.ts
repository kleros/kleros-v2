...
function getEmittedEvent(eventName: string, receipt: ContractTransactionReceipt): EventLog {
...
describe("Home Evidence contract", async () => {
  ...
  describe("Governance", async () => {
    it("Should change parameters correctly", async () => {
      ...
      const tx = await evidenceModule.changeDisputeTemplate(newDisputeTemplate, "disputeTemplateMapping: TODO");
      const receipt = await tx.wait();
      const lastArbitratorIndex = await evidenceModule.getCurrentArbitratorIndex();
      let newArbitratorData = await evidenceModule.arbitratorDataList(lastArbitratorIndex);
      const oldArbitratorData = await evidenceModule.arbitratorDataList(lastArbitratorIndex - 1n);
  ...
  describe("Evidence Submission", () => {
    it("Should submit evidence correctly.", async () => {
      ...
      const contributions = await evidenceModule.getContributions(evidenceID, 0, user1.address);
  ...
  describe("Moderation", () => {
    ...
    it("Should create dispute after moderation escalation is complete.", async () => {
      ...
      const receipt = await tx.wait();
      if (receipt === null) throw new Error("Receipt is null");
      const [_arbitrator, _disputeID, _templateId] = getEmittedEvent("DisputeRequest", receipt).args;
      expect(_arbitrator).to.equal(arbitrator.target, "Wrong arbitrator.");
      expect(_disputeID).to.equal(0, "Wrong dispute ID.");
