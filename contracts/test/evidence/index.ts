...
describe("Home Evidence contract", async () => {
  ...
  const totalCost = BigNumber.from(arbitrationFee)
    .mul(BigNumber.from(totalCostMultiplier))
    .div(MULTIPLIER_DIVISOR);
  const minRequiredDeposit = totalCost
    .mul(BigNumber.from(initialDepositMultiplier))
    .div(MULTIPLIER_DIVISOR);
  ...
  describe("Governance", async () => {
    it("Should change parameters correctly", async () => {
      ...
      await evidenceModule
        .connect(user2)
        .changeGovernor(await deployer.getAddress());
      ...
      let lastArbitratorIndex =
        await evidenceModule.getCurrentArbitratorIndex();
      let newArbitratorData = await evidenceModule.arbitratorDataList(
        lastArbitratorIndex
      );
      let oldArbitratorData = await evidenceModule.arbitratorDataList(
        lastArbitratorIndex.sub(BigNumber.from(1))
      );
      ...
      const [newMetaEvidenceUpdates, newMetaEvidence] = getEmittedEvent(
        "MetaEvidence",
        receipt
      ).args;
      expect(newMetaEvidence).to.equal(
        newMetaEvidenceUri,
        "Wrong MetaEvidence."
      );
      expect(newMetaEvidenceUpdates).to.equal(
        newArbitratorData.metaEvidenceUpdates,
        "Wrong MetaEvidence ID."
      );
      ...
      newArbitratorData = await evidenceModule.arbitratorDataList(
        lastArbitratorIndex.add(BigNumber.from(1))
      );
      expect(newArbitratorData.arbitratorExtraData).to.equal(
    ...
    it("Should revert if the caller is not the governor", async () => {
      await expect(
        evidenceModule.connect(user2).changeGovernor(await user2.getAddress())
      ...
      await expect(
        evidenceModule.connect(user2).changeMetaEvidence(metaEvidenceUri)
      ).to.be.revertedWith("The caller must be the governor");
  ...
  describe("Evidence Submission", () => {
    it("Should submit evidence correctly.", async () => {
      ...
      const evidenceID = ethers.utils.solidityKeccak256(
        ["uint", "string"],
        [1234, newEvidence]
      );

      const [arbitratorAddress, evidenceGroupID, submitter, evidenceStr] =
        getEmittedEvent("Evidence", receipt).args;
      expect(arbitratorAddress).to.equal(
        arbitrator.address,
        "Wrong arbitrator."
      );
      ...
      let contributions = await evidenceModule.getContributions(
        evidenceID,
        0,
        user1.address
      );
  ...
  describe("Moderation", () => {
    beforeEach("Initialize posts and comments", async () => {
      ...
      evidenceID = ethers.utils.solidityKeccak256(
        ["uint", "string"],
        [1234, newEvidence]
      );
    });
    ...
    it("Should create dispute after moderation escalation is complete.", async () => {
      ...
      let depositRequired = paidFees[Party.Moderator]
        .mul(2)
      ...
      depositRequired = paidFees[Party.Submitter]
        .mul(2)
      ...
      moderationInfo = await evidenceModule.getModerationInfo(evidenceID, 0);
      paidFees = moderationInfo.paidFees;
      depositRequired = paidFees[Party.Moderator]
        .mul(2)
        .sub(paidFees[Party.Submitter]);
      await evidenceModule
        .connect(user4)
      ...
      let [_arbitrator, disputeID, metaEvidenceID, _evidenceID] =
        getEmittedEvent("Dispute", receipt).args;
