...
import {
  deployERC1155,
  deployERC721,
} from "../../../deploy/utils/deployTokens";
...
export type DisputeKitGatedType =
  | DisputeKitGatedMock
  | DisputeKitGatedShutterMock;
...
const thousandPNK = (amount: BigNumberish) =>
  toBigInt(amount) * ONE_THOUSAND_PNK;
...
export const whitelistTokens = async (
  ...
  const isErc1155Included = tokenAddresses.some(
    (tokenAddress) => tokenAddress === nft1155Address,
  );
  const erc721LikeTokens = tokenAddresses.filter(
    (tokenAddress) => tokenAddress !== nft1155Address,
  );

  if (erc721LikeTokens.length > 0)
    await context.disputeKit.changeSupportedErc721Tokens(
      courtId,
      erc721LikeTokens,
      supported,
    );
  if (isErc1155Included)
    await context.disputeKit.changeSupportedErc1155TokenIds(
      courtId,
      nft1155Address,
      [context.TOKEN_ID],
      supported,
...
export const createDisputeWithToken = async (
  ...
  const extraData = encodeExtraData(
    courtId,
    3,
    context.gatedDisputeKitID,
    token,
    isERC1155,
    tokenId,
  );
  const arbitrationCost =
    await context.core["arbitrationCost(bytes)"](extraData);
...
export const expectTokenSupported = async (
  ...
  if (tokenId !== undefined || isKnownErc1155) {
    ...
    expect(
      await context.disputeKit.isErc1155TokenIdSupported(
        courtId,
        tokenAddress,
        effectiveTokenId,
      ),
  ...
  expect(
    await context.disputeKit.isErc721TokenSupported(courtId, tokenAddress),
  ).to.equal(supported);
};
...
export const stakeAndDraw = async (
  ...
  for (const juror of [context.juror1, context.juror2]) {
    await context.pnk
      .transfer(juror.address, context.thousandPNK(10))
      .then((tx) => tx.wait());
    expect(await context.pnk.balanceOf(juror.address)).to.equal(
      context.thousandPNK(10),
    );
    ...
    expect(
      await context.sortitionModule.getJurorBalance(juror.address, 1),
    ).to.deep.equal([
...
const isDrawEventLog = (
  log: Log | EventLog,
  coreAddress: string | Addressable,
): log is EventLog => {
  return (
    log instanceof EventLog &&
    log.fragment.name === "Draw" &&
    log.address === coreAddress
  );
};

// Helper functions to check if a log is of type EventLog and also a court created event
const isCourtCreatedEventLog = (
  log: Log | EventLog,
  coreAddress: string | Addressable,
): log is EventLog => {
  return (
    log instanceof EventLog &&
    log.fragment.name === "CourtCreated" &&
    log.address === coreAddress
  );
};
...
export async function setupTokenGatedTest(
  ...
  const deploymentResult = await deployUpgradable(
    deployments,
    config.contractName,
    {
      from: deployer,
  ...
  const disputeKit = await ethers.getContract<DisputeKitGatedType>(
    config.contractName,
  );
  ...
  await whitelistTokens(
    context,
    [dai.target, nft721.target, nft1155.target],
    true,
  );
...
export function testTokenWhitelistManagement(
  context: () => TokenGatedTestContext,
) {
  describe("Token Whitelist Management", async () => {
    describe("changeSupportedErc721Tokens / changeSupportedErc1155TokenIds functions", async () => {
      ...
      it("Should allow owner to whitelist multiple tokens", async () => {
        ...
        await whitelistTokens(
          ctx,
          [ctx.dai.target, ctx.nft721.target, ctx.nft1155.target],
          true,
        );
        await expectTokenSupported(ctx, ctx.dai.target, true);
  ...
  describe("Access Control", async () => {
    it("Should revert when non-owner tries to change supported ERC721-like tokens", async () => {
      ...
      await expect(
        ctx.disputeKit
          .connect(ctx.juror1)
          .changeSupportedErc721Tokens(Courts.GENERAL, [ctx.dai.target], true),
      ).to.be.reverted;
    });
...
export function testUnsupportedTokenErrors(
  context: () => TokenGatedTestContext,
) {
  describe("Error Handling - Unsupported Tokens", async () => {
    ...
    it("Should check token support using the courtID encoded in extraData", async () => {
      ...
      await ctx.core
        ...
        .enableDisputeKits(Courts.FORKING, [ctx.gatedDisputeKitID], true);
      ...
      await expect(
        createDisputeWithToken(ctx, ctx.dai.target, false, 0, Courts.FORKING),
      )
        .to.be.revertedWithCustomError(ctx.disputeKit, "TokenNotSupported")
    ...
    it("Should revert with TokenNotSupported when creating dispute with unsupported ERC1155", async () => {
      ...
      await expect(
        createDisputeWithToken(ctx, ctx.nft1155.target, true, ctx.TOKEN_ID),
      )
        .to.be.revertedWithCustomError(ctx.disputeKit, "TokenNotSupported")
    ...
    it("Should allow dispute creation after token is whitelisted", async () => {
      ...
      await expect(
        createDisputeWithToken(ctx, ctx.dai.target),
      ).to.be.revertedWithCustomError(ctx.disputeKit, "TokenNotSupported");

      await whitelistTokens(ctx, [ctx.dai.target], true);

      await expect(createDisputeWithToken(ctx, ctx.dai.target)).to.not.be
        .reverted;
    });
  });
}
...
export function testERC20Gating(context: () => TokenGatedTestContext) {
  describe("When gating with DAI token", async () => {
    it("Should draw no juror if they don't have any DAI balance", async () => {
      ...
      const drawLogs =
        tx?.logs.filter((log) => isDrawEventLog(log, ctx.core.target)) || [];
    ...
    it("Should draw only the jurors who have some DAI balance", async () => {
      ...
      expect(
        await ctx.sortitionModule.getJurorBalance(
          ctx.juror1.address,
          Courts.GENERAL,
        ),
      ).to.deep.equal([
...
export function testTokenGateRequired(context: () => TokenGatedTestContext) {
  describe("Token Gate Required (address(0) is invalid)", async () => {
    it("Should revert when whitelisting address(0)", async () => {
      const ctx = context();
      await expect(
        ctx.disputeKit.changeSupportedErc721Tokens(
          Courts.GENERAL,
          [ethers.ZeroAddress],
          true,
        ),
    ...
    it("Should revert when creating dispute with tokenGate = address(0)", async () => {
      const ctx = context();
      await expect(
        createDisputeWithToken(ctx, ethers.ZeroAddress, false, 0),
      ).to.be.revertedWithCustomError(ctx.disputeKit, "TokenGateRequired");
    });
    ...
    it("Should parse address(0) correctly from insufficient extraData", async () => {
      ...
      const tokenInfo =
        await ctx.disputeKit.extraDataToTokenInfo(shortExtraData);
...
export function testCourtEligibilityMisconfiguration(
  context: () => TokenGatedTestContext,
) {
  describe("Court eligibility misconfiguration (eligibility set before token config)", async () => {
    async function createCourtWithEligibility(
      ctx: TokenGatedTestContext,
    ): Promise<number> {
      const deployerSigner = await ethers.getSigner(ctx.deployer);
      ...
      const createdLog = tx?.logs.find((log) =>
        isCourtCreatedEventLog(log, ctx.core.target),
      );
    ...
    it("Should revert NotEligibleForStaking when eligibility is set but no supported tokens are configured", async () => {
      ...
      expect(
        await ctx.disputeKit.supportedErc721TokensLength(courtId),
      ).to.equal(0);
      expect(
        await ctx.disputeKit.supportedErc1155TokensLength(courtId),
      ).to.equal(0);

      await fundAndApprove(ctx, ctx.juror1, ctx.thousandPNK(10));
      await expect(
        ctx.core.connect(ctx.juror1).setStake(courtId, ctx.thousandPNK(10)),
      ).to.be.revertedWithCustomError(ctx.core, "NotEligibleForStaking");
    });

    it("Should not revert eligibility when ERC721 supported set contains address(0)", async () => {
      const ctx = context();
      const courtId = await createCourtWithEligibility(ctx);
      await ctx.disputeKit.unsafeAddSupportedErc721Token(
        courtId,
        ethers.ZeroAddress,
      );
      ...
      expect(await ctx.disputeKit.supportedErc721TokensAt(courtId, 0)).to.equal(
        ethers.ZeroAddress,
      );
      expect(
        await ctx.disputeKit.isEligible(ctx.juror1.address, courtId),
      ).to.equal(false);
    ...
    it("Should not revert eligibility when ERC1155 supported set contains address(0)", async () => {
      const ctx = context();
      const courtId = await createCourtWithEligibility(ctx);
      await ctx.disputeKit.unsafeAddSupportedErc1155Token(
        courtId,
        ethers.ZeroAddress,
      );
      ...
      expect(
        await ctx.disputeKit.supportedErc1155TokensAt(courtId, 0),
