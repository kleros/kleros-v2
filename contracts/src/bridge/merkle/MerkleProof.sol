// SPDX-License-Identifier: MIT

/**
 *  @authors: [@shotaronowhere]
 *  @reviewers: []
 *  @auditors: []
 *  @bounties: []
 *  @deployments: []
 */

pragma solidity ^0.8.0;

/**
 *  @title MerkleProof
 *  @author Shotaro N. - <shawtarohgn@gmail.com>
 *  @dev A set of funcitons to verify merkle proofs.
 */
contract MerkleProof {

    /** @dev Validates membership of leaf in merkle tree with merkle proof.
     *  @param proof The merkle proof.
     *  @param route The index of item in the merkle tree.
     *  @param leaf The leaf to validate membership in merkle tree.
     *  @param merkleRoot The root of the merkle tree.
     */
    function validateProof(
        bytes32[] memory proof,
        uint256 route,
        bytes32 leaf,
        bytes32 merkleRoot
    ) internal pure returns (bool){
        return(merkleRoot == calculateRoot(proof,route,leaf));
    }

    /** @dev Validates membership of leaf in merkle tree with merkle proof.
     *  @param proof The merkle proof.
     *  @param route The index of item in the merkle tree.
     *  @param data The data to validate membership in merkle tree.
     *  @param merkleRoot The root of the merkle tree.
     */
    function validateProof(
        bytes32[] memory proof,
        uint256 route,
        bytes memory data,
        bytes32 merkleRoot
    ) public pure returns (bool){
        return validateProof(proof, route, sha256(data), merkleRoot);
    }

    /** @dev Calculates merkle root from proof.
     *  @param proof The merkle proof.
     *  @param route The index of item in the merkle tree.
     *  @param leaf The leaf to validate membership in merkle tree..
     */
    function calculateRoot(
        bytes32[] memory proof,
        uint256 route,
        bytes32 leaf
    ) internal pure returns (bytes32) {
        uint256 proofLength = proof.length;
        require(proofLength <= 32, "Invalid Proof");
        bytes32 h = leaf;
        for (uint256 i = 0; i < proofLength; i++) {
            bytes32 evidence = proof[i];
            // effecient hash
            if (route % 2 == 0) {
                assembly {
                    mstore(0x00, h)
                    mstore(0x20, evidence)
                    h := keccak256(0x00, 0x40)
                }
            } else {
                assembly {
                    mstore(0x00, evidence)
                    mstore(0x20, h)
                    h := keccak256(0x00, 0x40)
                }
            }
            route /= 2;
        }
        return h;
    }
}
