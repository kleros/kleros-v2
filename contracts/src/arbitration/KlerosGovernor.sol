...
contract KlerosGovernor is IArbitrable, IMetaEvidence {
    ...
    modifier duringSubmissionPeriod() {
        ...
        require(
            block.timestamp - lastApprovalTime <=
                submissionTimeout.addCap(offset),
     ...
     */
    function changeSubmissionDeposit(
        uint256 _submissionBaseDeposit
     ...
     */
    function changeSubmissionTimeout(
        uint256 _submissionTimeout
     ...
     */
    function changeExecutionTimeout(
        uint256 _executionTimeout
    ) external onlyByGovernor {
        executionTimeout = _executionTimeout;
    }
    ...
    function submitList(
        ...
        require(
            ...
            "Wrong input: target and value"
        );
        require(
            _target.length == _dataSize.length,
            "Wrong input: target and datasize"
        );
        ...
        require(
            msg.value >= submission.deposit,
            "Not enough ETH to cover deposit"
        );
        ...
        for (uint256 i = 0; i < _target.length; i++) {
            ...
            require(
                uint256(hashes[2]) >= uint256(hashes[1]),
                "Incorrect tx order"
            );
     ...
     */
    function withdrawTransactionList(
        uint256 _submissionID,
        bytes32 _listHash
    ) external {
        Session storage session = sessions[sessions.length - 1];
        Submission storage submission = submissions[
            session.submittedLists[_submissionID]
        ];
        require(
            block.timestamp - lastApprovalTime <= submissionTimeout / 2,
        ...
        require(
            block.timestamp - submission.submissionTime <= withdrawTimeout,
            "Withdrawing time has passed."
        );
        session.submittedLists[_submissionID] = session.submittedLists[
            session.submittedLists.length - 1
        ];
    ...
    function executeSubmissions() external duringApprovalPeriod {
        ...
        if (session.submittedLists.length == 0) {
            ...
            session.disputeID = arbitrator.createDispute{
                value: arbitrationCost
            }(session.submittedLists.length, arbitratorExtraData);
    ...
    function rule(uint256 _disputeID, uint256 _ruling) external override {
        ...
        require(
            _ruling <= session.submittedLists.length,
            "Ruling is out of bounds."
        );

        if (_ruling != 0) {
            Submission storage submission = submissions[
                session.submittedLists[_ruling - 1]
            ];
            submission.approved = true;
            submission.approvalTime = block.timestamp;
    ...
    function executeTransactionList(
        ...
        require(
            block.timestamp - submission.approvalTime <= executionTimeout,
            "Time to execute has passed"
        );
        for (
            uint256 i = _cursor;
            i < submission.txs.length && (_count == 0 || i < _cursor + _count);
            i++
        ) {
            ...
            if (!transaction.executed && transaction.value <= expendableFunds) {
                (bool callResult, ) = transaction.target.call{
     ...
     */
    function getSubmittedLists(
        uint256 _session
    ) external view returns (uint256[] memory submittedLists) {
    ...
    function getNumberOfTransactions(
        uint256 _listID
    ) external view returns (uint256 txCount) {
        Submission storage submission = submissions[_listID];
