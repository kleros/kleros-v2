...
contract xKlerosLiquidV2 is Initializable, ITokenController, IArbitrator {
    ...
    function changeRNGenerator(
        ...
        if (phase == Phase.generating) {
            RNBlock =
                RNGenerator.nextCommitPhaseStartBlock() +
                RNGenerator.collectRoundLength();
        }
    }
    ...
    function executeDelayedSetStakes(
        ...
        uint256 actualIterations = (nextDelayedSetStake + _iterations) - 1 >
            lastDelayedSetStake
    ...
    function rule(uint256 _disputeID, uint256 _ruling) external {
        ...
        require(
            msg.sender == address(foreignGateway),
            "Can only be called by gateway"
        );
    ...
    function createDispute(
        ...
        require(
            weth.transferFrom(msg.sender, address(this), fee),
            "Not enough WETH for arbitration"
        );
        ...
        bytes memory extraDataV2 = abi.encode(
            uint256(subcourtID + 1),
            minJurors
        );

        require(
            weth.transfer(address(foreignGateway), fee),
            "Fee transfer to gateway failed"
        );
    ...
    function proxyPayment(
        address _owner
    ) public payable override returns (bool allowed) {
        allowed = false;
    }
    ...
    function onTransfer(
        ...
        address _to,
        uint256 _amount
    ) public override returns (bool allowed) {
    ...
    function currentRuling(
        uint256 _disputeID
    ) public view returns (uint256 ruling) {
        Dispute storage dispute = disputes[_disputeID];
    ...
    function _setStake(
        ...
        uint256 currentStake = sortitionSumTrees.stakeOf(
        ...
        returns (uint256 winningChoice, uint256[] memory counts, bool tied)
    {
        ...
        for (uint256 i = 0; i <= dispute.numberOfChoices; i++)
    ...
    function getJuror(
        address _account
    ) external view returns (uint96[] memory subcourtIDs) {
        Juror storage juror = jurors[_account];
