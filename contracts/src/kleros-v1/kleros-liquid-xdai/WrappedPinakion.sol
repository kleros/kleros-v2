...
contract WrappedPinakion is Initializable {
    ...
    function onTokenBridged(
        ...
        require(
            _token == address(xPinakion),
            "Token bridged is not xPinakion."
        );
    ...
    function withdraw(uint256 _amount) external {
        _burn(_amount);
        require(
            xPinakion.transfer(msg.sender, _amount),
     ...
     */
    function withdrawAndConvertToPNK(
        uint256 _amount,
        address _receiver
    ) external {
        _burn(_amount);
    ...
    function transfer(
        ...
        if (isContract(controller)) {
            require(
                ITokenController(controller).onTransfer(
                    msg.sender,
                    _recipient,
                    _amount
                ),
    ...
    function transferFrom(
        ...
        if (msg.sender != controller) {
            allowance[_sender][msg.sender] = allowance[_sender][msg.sender].sub(
                _amount
    ...
    function decreaseAllowance(
        ...
        uint256 newAllowance = allowance[msg.sender][_spender].sub(
            _subtractedValue
