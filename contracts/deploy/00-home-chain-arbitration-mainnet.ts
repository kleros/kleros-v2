...
const deployArbitration: DeployFunction = async (
  ...
  const nft = await deployERC721(
    hre,
    deployer,
    "Kleros V2 Neo Early User",
  ...
  const minStakingTime = devnet
    ? 3 * ONE_MINUTE_IN_SECONDS
    : 30 * ONE_MINUTE_IN_SECONDS;
  const maxFreezingTime = devnet
  ...
  const disputeKitContract =
    await hre.ethers.getContract<DisputeKitClassic>("DisputeKitClassic");
