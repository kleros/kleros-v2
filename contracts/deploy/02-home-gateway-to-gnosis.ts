...
const deployHomeGateway: DeployFunction = async (
  ...
  const foreignGateway =
    await hre.companionNetworks.foreignChiado.deployments.get(
      "ForeignGatewayOnGnosis",
    );
  const foreignChainId = Number(
    await hre.companionNetworks.foreignChiado.getChainId(),
  );
  const foreignChainName =
    await hre.companionNetworks.foreignChiado.deployments.getNetworkName();
  console.log(
