...
const deployArbitration: DeployFunction = async (
  ...
  const oldDisputeKit = (await ethers.getContract(
    "DisputeKitClassic",
  ...
  const newDisputeKit = (await ethers.getContract(
    "DisputeKitClassic",
  )) as DisputeKitClassic;
  ...
  await execute(
    ...
    "changeCore",
    await klerosCore.getAddress(),
  );
  await execute(
    "KlerosCore",
    { from: deployer, log: true },
    "addNewDisputeKit",
    await newDisputeKit.getAddress(),
    0,
  );
  ...
  assert(
    await klerosCore
      .disputeKits(oldDisputeKitId)
      .then((dk) => dk === oldDisputeKitAddress),
    `wrong dispute kit id ${oldDisputeKitId}`,
  );
  assert(
    await klerosCore
      .disputeKits(newDisputeKitId)
      .then((dk) => dk === newDisputeKitAddress),
