...
const deployForeignGateway: DeployFunction = async (
  ...
  const homeNetwork = config.networks[
    network.companionNetworks.home
  ...
  const coreDeployment =
    await hre.companionNetworks.home.deployments.get("KlerosCore");
  const core = await KlerosCore__factory.connect(
