...
const deployHomeGateway: DeployFunction = async (
  ...
  const foreignGateway =
    await hre.companionNetworks.foreignSepolia.deployments.get(
      "ForeignGatewayOnEthereum",
    );
  const foreignChainId = Number(
    await hre.companionNetworks.foreignSepolia.getChainId(),
  );
  const foreignChainName =
    await hre.companionNetworks.foreignSepolia.deployments.getNetworkName();
  console.log(
    "using ForeignGateway %s on chainId %s (%s)",
...
deployHomeGateway.skip = async ({ network }) => {
  ...
  return isSkipped(
    network,
    !HomeChains[chainId] || HardhatChain[chainId] !== undefined,
  );
};
