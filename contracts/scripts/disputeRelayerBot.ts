...
export default async function main(
  ...
  const disputeRequestTopic = arbitrableInterface.getEvent("DisputeRequest").topicHash;
  ...
  foreignGateway.on(
    ...
    async (foreignBlockHash, foreignArbitrable, foreignDisputeID, choices, extraData, eventLog) => {
      ...
      const fullTxReceipt = await foreignChainProvider.getTransactionReceipt(eventLog.transactionHash);
      if (!fullTxReceipt) {
        throw new Error(`No transaction receipt for ${eventLog.transactionHash}`);
      }

      // Retrieve the DisputeRequest event
      const disputeRequests: DisputeRequestEvent.OutputObject[] = fullTxReceipt.logs
        .filter((log: Log) => log.topics[0] === disputeRequestTopic)
        .map((log: Log) => {
          const parsed = arbitrableInterface.parseLog(log);
          if (!parsed) {
            throw new Error(`Failed to parse DisputeRequest log: ${log.transactionHash}`);
          }
      ...
      logger.info(`Relaying dispute to home chain... ${safeJson(relayCreateDisputeParams)}`);

      let tx;
      if (feeToken === undefined) {
        // Paying in native Arbitrum ETH
        const cost = await core["arbitrationCost(bytes)"](extraData);
        tx = await homeGateway["relayCreateDispute((bytes32,uint256,address,uint256,uint256,uint256,bytes))"](
          relayCreateDisputeParams,
          { value: cost }
        );
      } else {
        // Paying in ERC20
        const cost = await core["arbitrationCost(bytes,address)"](extraData, feeToken.target);
        await (await feeToken.approve(homeGateway.target, cost)).wait();
        tx = await homeGateway["relayCreateDispute((bytes32,uint256,address,uint256,uint256,uint256,bytes),uint256)"](
          relayCreateDisputeParams,
          cost
        );
      }
