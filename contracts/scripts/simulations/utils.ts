...
export const getContracts = async (hre) => {
  ...
  const disputeKitClassic = (await hre.ethers.getContract(
    "DisputeKitClassic"
  )) as DisputeKitClassic;
  const pnk = (await hre.ethers.getContract("PNK")) as PNK;
  const randomizerRng = (await hre.ethers.getContract(
    "RandomizerRNG"
  )) as RandomizerRNG;
  const arbitrable = (await hre.ethers.getContract(
    "ArbitrableExampleEthFee"
  )) as ArbitrableExampleEthFee;
  const randomizerMock = (await hre.ethers.getContract(
    "RandomizerMock"
  )) as RandomizerMock;
...
export const isRngReady = async (wallet, hre) => {
  ...
  const requesterID = await randomizerRng
    .connect(wallet)
    .requesterToID(disputeKitClassic.address);
...
export const getArbitrationFees = (
  hre,
  nbofjurors: bigint,
  feeforjuror: bigint
) => {
...
export const findVoteIdInDrawnJurors = (
  walletAddress: string,
  drawnJurors: string[]
) => {
  return drawnJurors.findIndex(
...
export const waitForPeriod = async (disputeId: number, period: Period, hre) => {
  ...
  const remainingDuration = lastPeriodChange
    .add(periodDuration)
    .sub(now)
    .toNumber();
