...
task(
  "simulate:approve",
  "Grants permission to KlerosCore to use your PNK tokens"
)
  .addParam(
    "walletindex",
    "Index of the wallet to call this function from and approve it"
  )
  .addParam(
    "pnkamount",
    "(in normal values, not wei!) Amount of PNK tokens to approve for transfer"
  )
  .setAction(async (taskArgs, hre) => {
    ...
    try {
      const approvePNKFunctionArgs: [string, BigNumber] = [
        core.address,
        utils.parseEther(pnkamount.toString()),
      ];
      const resultApproveTx = await pnk
        .connect(wallet)
        .approve(...approvePNKFunctionArgs, options);
      await resultApproveTx.wait();
      console.log(
        `Approved ${pnkamount} PNK tokens for transfer to KlerosCore from wallet ${wallet.address}`
      );
...
task(
  "simulate:set-stake",
  "Stakes PNK tokens to participate in the Kleros dispute resolution"
)
  .addParam(
    "walletindex",
    "Index of the wallet to call this function from and stake it"
  )
  .addParam(
    "pnkamount",
    "(in normal values, not wei!) Amount of PNK tokens to stake"
  )
  .addParam(
    "courtid",
    "the ID of the court you want to stake on (General Court ID = 1"
  )
  .setAction(async (taskArgs, hre) => {
    ...
    try {
      const setStakeFunctionArgs: [string, BigNumber] = [
        courtid,
        utils.parseEther(pnkamount.toString()),
      ];
      const resultSetStakeTx = await core
        .connect(wallet)
        .setStake(...setStakeFunctionArgs, options);
...
task("simulate:create-dispute", "Creates a dispute on an arbitrable contract")
  .addParam(
    "walletindex",
    "Index of the wallet to use for creating the dispute"
  )
  .addParam("courtid", "ID of the court to create the dispute in")
  .addParam("nbofchoices", "Number of choices for the dispute")
  .addParam(
    "nbofjurors",
    "Number of jurors for the dispute (in BigInt) (ex: 3n)"
  )
  .addParam(
    "feeforjuror",
    "Fee for each juror in wei (in BigInt) (ex: 100000n)"
  )
  .setAction(async (taskArgs, hre) => {
    const { arbitrable, core } = await getContracts(hre);
    const { walletindex, courtid, nbofchoices, nbofjurors, feeforjuror } =
      taskArgs;
    const { ethers } = hre;
    ...
    const evidenceGroupID = ethers.BigNumber.from(
    ...
    const courtIdBytes = ethers.utils.defaultAbiCoder.encode(
      ["uint256"],
      [courtid]
    );
    const minJurorsBytes = ethers.utils.defaultAbiCoder.encode(
      ["uint256"],
      [3]
    ); // default value for minimum jurors

    const arbitratorExtraData = ethers.utils.hexConcat([
      courtIdBytes,
      minJurorsBytes,
    ]);
    ...
    try {
      ...
      const tx = await arbitrable
        .connect(wallet)
        .createDispute(nbofchoices, arbitratorExtraData, evidenceGroupID, {
      ...
      console.log(
        "Created Dispute %s on Court %s with evidenceGroupID %s",
        disputeID,
        courtid,
        evidenceGroupID
      );
...
task(
  "simulate:pass-phase-dk",
  "Pass the phase of the DisputeKitClassic contract"
)
  .addParam("walletindex", "Index of the wallet to use for passing the phase")
  .setAction(async (taskArgs, hre) => {
...
task("simulate:wait-for-rng", "Waits for the RNG to be ready").setAction(
  async (taskArgs, hre) => {
    const walletIndex = 0;
    const { wallet } = await getWallet(hre, walletIndex);
...
task("simulate:draw", "Draws jurors for a dispute on Kleros")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    try {
      const tx = await (
        await core.connect(wallet).draw(disputeid, 10, options)
      ).wait();
...
task("simulate:pass-period", "Passes the period of a dispute on Kleros Core")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    try {
      const tx = await (
        await core.connect(wallet).passPeriod(disputeid, options)
      ).wait();
...
task("simulate:cast-commit", "Casts a commit for a drawn juror")
  ...
  .addParam(
    "justification",
    "The justification of making that choice, not mandatory"
  )
  .setAction(async (taskArgs, hre) => {
    const { disputeKitClassic } = await getContracts(hre);
    ...
    const castCommitFunctionArgs: [string, string[], string] = [
      disputeid,
      voteIdsParam,
      commitHash,
    ];
    const tx = await disputeKitClassic
      .connect(wallet)
      .castCommit(...castCommitFunctionArgs);
    await tx.wait();

    console.log(
      "juror %s casted a commit on txID: %s",
      wallet.address,
      tx?.hash
    );
  });
...
task("simulate:cast-vote", "Casts a vote for a drawn juror")
  ...
  .addParam(
    "justification",
    "if there was a commit phase this must match the justification too"
  )
  .addOptionalParam(
    "salt",
    "the salt used for the commit ONLY IF there was a commit period"
  )
  .setAction(async (taskArgs, hre) => {
    const { disputeKitClassic } = await getContracts(hre);
    const { disputeid, walletindex, choice } = taskArgs;
...
task("simulate:fund-appeal", "Funds an appeal on a dispute")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    try {
      ...
      const fundAppealTx = await (
        await disputeKitClassic
          .connect(wallet)
          .fundAppeal(...fundAppealFunctionArgs)
      ).wait();
      console.log(
        "fundAppeal (in DisputeKitClassic) txID: %s",
        fundAppealTx?.transactionHash
      );
    } catch (e) {
      handleError(e);
    }
...
task(
  "simulate:execute-ruling",
  "Executes the ruling for a dispute on KlerosCore"
)
  .addParam(
    "walletindex",
    "Index of the wallet to use for executing the ruling"
  )
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    try {
      const executeTx = await (
        await connectedKlerosCore.execute(disputeid, 0, 10)
      ).wait(); // redistribute
      console.log("txID execute: %s", executeTx?.transactionHash);

      executeRulingTx = await (
        await connectedKlerosCore.executeRuling(disputeid)
      ).wait(); // rule
...
task(
  "simulate:withdraw-fees-and-rewards",
  "Withdraws fees and rewards for people who appealed and won"
)
  .addParam(
    "walletindex",
    "Index of the wallet to use for executing the ruling"
...
task(
  "simulate:stake-five-jurors",
  "Approve and stake 5 different wallets on the court 1"
)
  .addParam(
    "walletindexes",
    "An array of wallet indexes that you want to stake (ex: 0,1,2,3,4)"
  )
  .addParam(
    "pnkamounts",
    "An array of pnkamounts to stake (in normal values, not wei!) (ex: 200,300,200,100,200)"
  )
  .addParam(
    "courtid",
    "the ID of the court you want to stake on (General Court ID = 1"
  ...
  .setAction(async (taskArgs, hre) => {
    const walletIndexes = taskArgs.walletindexes
      .split(",")
      .map((index) => index.toString());
    const pnkAmounts = taskArgs.pnkamounts
      .split(",")
      .map((amount) => amount.toString());
    const { courtid } = taskArgs;

    await hre.run("simulate:approve", {
      walletindex: walletIndexes[0],
      pnkamount: pnkAmounts[0],
    });
    await hre.run("simulate:approve", {
      walletindex: walletIndexes[1],
      pnkamount: pnkAmounts[1],
    });
    await hre.run("simulate:approve", {
      walletindex: walletIndexes[2],
      pnkamount: pnkAmounts[2],
    });
    await hre.run("simulate:approve", {
      walletindex: walletIndexes[3],
      pnkamount: pnkAmounts[3],
    });
    await hre.run("simulate:approve", {
      walletindex: walletIndexes[4],
      pnkamount: pnkAmounts[4],
    });

    await hre.run("simulate:set-stake", {
      walletindex: walletIndexes[0],
      pnkamount: pnkAmounts[0],
      courtid: courtid,
    });
...
task(
  ...
  .setAction(async (taskArgs, hre: any) => {
    ...
    if (isNetworkLocal(hre)) {
      const { disputeKitClassic, randomizerMock, randomizerRng } =
        await getContracts(hre);
      const { wallet } = await getWallet(hre, walletindex);
      const numberOfBlocksToMine = Number(
        await disputeKitClassic.connect(wallet).rngLookahead()
      );
      await mineBlocks(numberOfBlocksToMine, hre.network);
      await randomizerMock
        .connect(wallet)
        .relay(randomizerRng.address, 0, utils.randomBytes(32));
    }
  });
...
task(
  ...
  .addParam("disputeid", "The ID of the dispute to draw jurors for")
  .setAction(async (taskArgs, hre) => {
    await hre.run("simulate:pass-phase-dk", {
      walletindex: taskArgs.walletindex,
    });
    await hre.run("simulate:draw", {
      walletindex: taskArgs.walletindex,
      disputeid: taskArgs.disputeid,
    });
    await hre.run("simulate:pass-phase-dk", {
      walletindex: taskArgs.walletindex,
    });
    await hre.run("simulate:pass-phase-core", {
      walletindex: taskArgs.walletindex,
    });
  });
...
task("simulate:split-pnk", "one wallet funds with PNK the other 4 wallets")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const firstTx = await pnk
      .connect(wallet)
      .transfer(firstWallet.address, amount);
    console.log(
      "funded wallet %s with %s: %s",
      firstWallet.address,
      amount,
      firstTx?.hash
    );
    const secondTx = await pnk
      .connect(wallet)
      .transfer(secondWallet.address, amount);
    console.log(
      "funded wallet %s with %s: %s",
    ...
    const thirdTx = await pnk
      .connect(wallet)
      .transfer(thirdWallet.address, amount);
    console.log(
      "funded wallet %s with %s: %s",
      thirdWallet.address,
      amount,
    ...
    const fourthTx = await pnk
      .connect(wallet)
      .transfer(fourthWallet.address, amount);
    console.log(
      "funded wallet %s with %s: %s",
...
task("simulate:submit-evidence", "Submits evidence to a dispute")
  ...
  .addParam(
    "evidencegroupid",
    "The evidenceGroupID, which is linked to the dispute"
  )
  .setAction(async (taskArgs, hre) => {
    const { disputeKitClassic } = await getContracts(hre);
    ...
    const { wallet } = await getWallet(hre, walletindex);
    const evidence =
      "/ipfs/QmaNEknCUEKDy74sDNLFXo8Vc3PqX7QC1ZyvUx4n7V6q1L/evidence.json";
    try {
      const tx = await (
        await disputeKitClassic
          .connect(wallet)
          .submitEvidence(evidencegroupid, evidence)
      ).wait();
...
task(
  "simulate:all",
  "Simulates arbitration activity to troubleshoot the subgraph and frontend"
).setAction(async (taskArgs, hre) => {
  const { core } = await getContracts(hre);
  const operator = 0;
  const stake = 1000;
  ...
  await hre.run("simulate:split-pnk", {
    walletindex: operator.toString(),
    pnkperwallet: stake.toString(),
  });
  ...
  await hre.run("simulate:to-freezing-and-generating-phase", {
    walletindex: operator.toString(),
  });
  ...
  await hre.run("simulate:pass-period", {
    walletindex: operator.toString(),
    disputeid: dispute.id.toString(),
  });
  ...
  const drawnJurors = await getDrawnJurors(
    hre,
    dispute.id,
    getLatestRoundId(hre, dispute.id)
  );
  const firstDrawnJurorWalletIndex = (await findFirstDrawnJurorWalletIndex(
    hre,
    drawnJurors
  )) as number;
  ...
  // Execution
  await hre.run("simulate:execute-ruling", {
    walletindex: operator.toString(),
    disputeid: dispute.id.toString(),
  });
});
