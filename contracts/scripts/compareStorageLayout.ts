...
task(
  "compare-storage",
  "Compare storage layout between deployed and modified contracts",
)
  .addParam("contract", "The name of the contract to compare against")
  .addOptionalParam(
    "deployedArtifact",
    "The name of the deployed contract artifact in deployments",
  )
  .setAction(
    async ({ deployedArtifact, contract }, hre: HardhatRuntimeEnvironment) => {
      try {
        // Get storage layout of the deployed contract implementation
        const artifactName = deployedArtifact || `${contract}_Implementation`;
        const deployedArtifactPath = path.resolve(
          `deployments/${hre.network.name}/${artifactName}.json`,
        );
        const deployedJson = JSON.parse(
          readFileSync(deployedArtifactPath, "utf8"),
        );
        ...
        const contractFile = Object.keys(
          buildInfoContent.output.contracts,
        ).find((file) =>
          Object.keys(buildInfoContent.output.contracts[file]).includes(
            contract,
          ),
        );
        ...
        const contractData =
          buildInfoContent.output.contracts[contractFile][contract];
