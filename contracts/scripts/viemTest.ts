...
const main = async () => {
  ...
  const getFunctionReturnParameterNames = (
    ...
    const f = abi.filter(
      (abi: AbiFunction) => abi.type === "function" && abi.name === name,
    )[0]; // WARNING: overloaded functions confusion
    return f.outputs.map((item) => item.name).filter(String) as string[];
  };

  const createObject = (keys: string[], values: unknown[]) =>
    Object.fromEntries(keys.map((k, i) => [k, values[i]]));

  const disputes = await disputeKit.read.disputes([BigInt(0)]);
  const disputeParamNames = getFunctionReturnParameterNames(
    disputeKit.abi as unknown as AbiFunction[],
    "disputes",
  ); // such type hack
