  ...
  .setAction(
    async (
      ...
      const disputeTemplateRegistry =
        await ethers.getContract<IDisputeTemplateRegistry>(
      ...
      let events;
      try {
        const filter = disputeTemplateRegistry.filters.DisputeTemplate(
          BigInt(templateId),
        );
        ...
        console.error(
          `Failed to query events for template ID ${templateId}:`,
          errorMessage,
        );
        return;
      }
      ...
      console.log(
        `Template Data Mappings: ${event.args._templateDataMappings}`,
      );
    },
  );
