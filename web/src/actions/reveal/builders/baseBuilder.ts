...
/**
 * Typed factory for Reveal action builders.
 *
 * This binds the generic {@link defineActionBuilder} helper to:
 * - {@link RevealParams}  – parameters supplied by the caller
 * - {@link RevealContext} – execution context (account, chain, wallet client)
 *
 *
 * Individual reveal builders may still:
 ...
 *
 * @remarks
 * Reveal actions do not require any shared or global dependencies, so an empty
 ...
 */
export const defineRevealBuilder = defineActionBuilder<RevealParams, RevealContext, object>({});

/**
 * Concrete RevealBuilder type produced by {@link defineRevealBuilder}.
 *
 * Each reveal builder exposes a `build(params, context, deps?)` function that
 * returns a fully-typed transaction object ready to be submitted via a wallet
 * client.
 */
